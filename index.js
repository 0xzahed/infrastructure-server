...
app.patch("/issues/:id", ensureDB, verifyToken, async (req, res) => {
  try {
    ...
    if (issue.status !== "pending") {
      return res
        .status(400)
        .json({ message: "Only pending issues can be edited" });
    }
...
app.delete("/issues/:id", ensureDB, verifyToken, async (req, res) => {
  try {
    ...
    if (issue.status !== "pending") {
      return res
        .status(400)
        .json({ message: "Only pending issues can be deleted" });
    }
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const statusStats = await issuesCollection
...
app.patch(
  ...
  async (req, res) => {
    try {
      ...
      await usersCollection.updateOne({ email }, { $set: { isBlocked } });
...
app.patch("/users/:email/premium", ensureDB, verifyToken, async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Premium subscription activated successfully" });
  } catch (error) {
    res.status(500).json({ message: "Server error" });
  }
});
